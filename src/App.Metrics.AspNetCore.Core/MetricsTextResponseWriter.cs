// <copyright file="MetricsTextResponseWriter.cs" company="Allan Hardy">
...
{
    public class MetricsTextResponseWriter : IMetricsTextResponseWriter
    {
        private readonly MetricsOptions _metricsOptions;

        public MetricsTextResponseWriter(
        ...
        public Task WriteAsync(HttpContext context, MetricsDataValueSource metricsData, CancellationToken token = default(CancellationToken))
        {
            ...
            if (acceptHeaderMediaType.Accept != null)
            {
                ...
                {
                    formatter = ResolveFormatter(_metricsOptions.OutputMetricsTextFormatters, accept);
            ...
            if (formatter == default(IMetricsOutputFormatter))
            {
                if (_metricsOptions.DefaultMetricsTextOutputFormatter == default(IMetricsOutputFormatter))
                {
                    context.Response.StatusCode = StatusCodes.Status406NotAcceptable;
                    return Task.CompletedTask;
                }

                formatter = _metricsOptions.DefaultMetricsTextOutputFormatter;
            }
